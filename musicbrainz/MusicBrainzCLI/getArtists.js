...
module.exports = {
  getArtists: function(finalArray) {
 ...
 intervalId3 = setInterval(function () {
        ...
        if (elemNum == finalArray.length) {
            ...
            for (i = 0; i < obj.table.length; i++) {
              if (obj.table[i].hasOwnProperty("aboutartist")) {
                ...
                if (aboutArtist.hasOwnProperty("area") && aboutArtist.area != null) {
                    newObj3.placeOfBirth = aboutArtist.area.name;
                }
            ...
            fs.writeFile('BrainzArtistsSequelize.json', json2, 'utf8', function writeFileCallback(err, data) {
                process.send("done from Artists");
                process.exit();
            }); // write it back
